...
- 生成設定には24 FPSの推奨キャンバスと、124 / 243 / 345フレーム（約5.17 / 10.13 / 14.38秒）を用意しています。Latent 2-passは初期値OFFです。固定SHAの3D Conv v1重みを検証できた場合だけ選択でき、実測済み既定値は強さ0.18／再精製4 evaluationsです。512×512と標準960×544の124-frame実生成で24ch正規化、画質、音声、metadataを確認しています。
